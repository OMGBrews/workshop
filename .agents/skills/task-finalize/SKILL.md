...
1. **Record the inspected commit**: `git rev-parse HEAD`. Phase 6 derives the conservative baseline it stamps from this SHA. If `git status --porcelain -- <scoped paths>` reports uncommitted changes, the tree you are about to verify is not any commit the stamp can describe — say so, and either get that state committed first or carry the caveat into the Phase 9 report. (The task file's own pre-run state is Phase 1's snapshot, not this check.)
...
4. **Classify history before constructing a range.** For a `finalized-at:` value, run `bash "$history_helper" check "$task_repo" "$inspected_commit" "$finalized_at"`. `STATUS=usable` permits `git log --oneline <baseline>..HEAD -- <scoped paths>` and a matching `git diff`; `STATUS=reverify` means verify every claim, cited construct, recommended-solution premise, and acceptance criterion directly against current files, while stating the helper's reason. Never retry the unusable SHA through a fallback. For an unstamped brief, classify its creating commit with the same helper before using it; an uncommitted brief has no vintage and relies entirely on direct verification. A helper execution error must be reported; continue with direct verification only where current files remain inspectable, otherwise stop. Never substitute a date (`--since`), a nearby commit, or the commit that edited the stamp. Skim any usable-range commit that plausibly touches the task's claims.
...
5. Select and stamp the comparison baseline. Run `bash "$history_helper" select "$task_repo" "$inspected_commit"`. The helper discovers the authoritative default remote through its unique remote-HEAD symbolic ref, fetches that branch, and returns the unique merge-base shared by the fetched tip and inspected commit. Set `finalized-at:` to its `BASELINE`. A conservative baseline may include already verified branch work in later comparisons; that deliberate repetition avoids losing post-finalization changes when branch commits are squash-merged, rebased, or amended. If discovery, fetch, complete history, or a unique merge-base is unavailable, the helper returns the inspected commit with `DURABILITY=limited`: stamp it, state the reason and that every consumer must fully re-verify if it becomes unusable. Do not guess a default branch, trust a stale tracking ref, create refs/hooks, amend commits, or repair credentials. Report the inspected commit and recorded baseline separately; the stamp never certifies uncommitted scoped changes.
...
- The Phase 3 verification result: what had drifted and was corrected (with the commits responsible), or "no drift found". Name both the inspected commit and recorded `finalized-at` baseline, disclose limited durability when selected, and carry any Phase 3 caveat (the tree held uncommitted scoped changes at verification time).
