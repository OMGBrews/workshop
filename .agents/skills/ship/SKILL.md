...
**If the PR probe fails, stop here — before the checks — and put it to the maintainer.** The fix is theirs to run: authenticating `gh` needs a credential this session does not hold, so ask for it as a command in their own shell (`! gh auth login --with-token`, which reads the token from stdin — they paste it and press Ctrl-D), then re-probe and continue. Ask for the token form rather than the interactive device flow: GitHub issues at most ten OAuth tokens per user and application, so on a fleet of containers each device-flow login silently revokes the oldest, which is some other container's working credential. Stopping now costs no throughput — the session blocks on the human either way — and leaves nothing half-shipped while it waits. If the maintainer instead says to just push, that is an ordinary instruction: push, hand back each repo's compare URL, and report "branch pushed, PR not opened" — never "shipped".
